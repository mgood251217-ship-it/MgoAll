...
$sql = "
    ...
    WHERE oi.calculator_id = ? AND oi.store_id = ?

";

$stmt = $koneksi->prepare($sql);
$stmt->bind_param("ii", $cal_id, $store_id);
$stmt->execute();
...
while ($row = $result->fetch_assoc()) {
    ...
    $items[] = [
        'calculator_items_id'  => $row['calculator_items_id'],
...
// Ambil total order
$stmt = $koneksi->prepare("SELECT total FROM calculator WHERE calculator_id = ? AND store_id = ?");
$stmt->bind_param("ii", $cal_id, $store_id);
$stmt->execute();
...
foreach ($items as $item) {
    $diskon = 0;
    $stmtDiskon = $koneksi->prepare("SELECT diskon FROM diskon_calculator_items WHERE calculator_id = ? AND product_id = ?");
    $stmtDiskon->bind_param("ii", $cal_id, $item['product_id']);
    $stmtDiskon->execute();
