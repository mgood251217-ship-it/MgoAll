...
$finishing_jersey = $_POST['finishingJersey'] ?? [];
$ukuran_jersey = $_POST['ukuranJersey'] ?? '';
...
if (!empty($finishing_jersey)) {
    $priceFinishingJersey = 0;
    foreach ($finishing_jersey as $jersey) {
        $priceFinishingJersey += getProductPrice((int)$jersey, $store_id, $koneksi);
    }
    $unit_price += $priceFinishingJersey;
    
}

if (!empty($ukuran_jersey)) {
    if ($ukuran_jersey === '5XL') {
        $unit_price += 50000;
    } elseif ($ukuran_jersey === '4XL') {
        $unit_price += 40000;
    } elseif ($ukuran_jersey === '3XL') {
        $unit_price += 30000;
    } elseif ($ukuran_jersey === '2XL') {
        $unit_price += 20000;
    } elseif ($ukuran_jersey === 'XL') {
        $unit_price += 10000;
    }
}
