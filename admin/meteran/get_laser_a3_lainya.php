...
// KARTU NAMA: Grup "KN" dan "KN BB"
$kn_categories = ['KN', 'KN BB'];
$product_data_kn = [
    'KN' => 0,
    'KN BB' => 0
];
...
if (!empty($order_ids)) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if (str_contains($name, 'KN') && str_contains($name, 'BB')) {
            $product_data_kn['KN BB'] += $qty;
        } elseif (str_contains($name, 'KN')) {
            $product_data_kn['KN'] += $qty;
        }
    }
...
if (empty($order_ids)) {
    ...
    // Query untuk total qty semua produk tipe STAMP
    $queryStr = "
    ...
    // Bind params dinamis (note: hanya order_ids)
              ...
              <div class="table-responsive mb-3">
                <table class="table table-bordered">
                  ...
                  <tbody>
                    ...
                    <?php if (!empty($product_data_kn)): ?>
                      <?php foreach ($product_data_kn as $name => $qty): ?>
                        <tr><td><?= htmlspecialchars($name) ?></td><td><?= $qty ?></td></tr>
                      <?php endforeach; ?>
                    <?php endif; ?>
