...
function formatKeInternasional($nomor) {
    ...
    // 2. Cek jika diawali '0' (misal: 0812...)
    if (strpos($nomor, '0') === 0) {
        $nomor = '62' . substr($nomor, 1);
    } 
    // 3. Jika sudah 62 tapi tanpa '+', biarkan tetap 62
...
function tampilkanTabelOrders(array $orders, $koneksi, array $users, string $role, string $sistim): void {
    ...
    if (!empty($userIds)) {
        ...
        $typesUser = str_repeat('i', count($userIds));

        $stmtUser = $koneksi->prepare("SELECT user_id, initial FROM users WHERE user_id IN ($placeholdersUser)");
        $stmtUser->bind_param($typesUser, ...$userIds);
        $stmtUser->execute();
        $resultUser = $stmtUser->get_result();

        while ($row = $resultUser->fetch_assoc()) {
            $userInitials[$row['user_id']] = $row['initial'];
        }
        $stmtUser->close();
    }
        ...
        <table class="table table-striped table-bordered table-smaller order-table <?= $sistim == 'OFFLINE' ? 'table-prim' : 'table-dan' ?>">
            ...
            <tbody>
                <?php foreach ($orders as $no => $order):
                    ...
                    $initial = $userInitials[$projectUser] ?? '';
                        ...
                        <div class="btn-group-row">
                            ...
                            <div class="btn-group">
                                ...
                                <ul class="dropdown-menu">
                                    <li>
                                        <a class="dropdown-item" onclick="printStruk(<?= $orderId ?>, <?= $order['store_id'] ?>)">
                                            Print langsung
                                        </a>
                                    </li>
                                    <li>
                                        <a class="dropdown-item" onclick="printStrukPDF(<?= $orderId ?>, <?= $order['store_id'] ?>)">
                                            Print PDF
                                        </a>
                                    </li>
                                </ul>
