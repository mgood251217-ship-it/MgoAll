...
if ($action !== 'export_csv' && $action !== 'import_csv') {
...
if ($action === 'add_stock') {
    ...
    $stmt = $koneksi->prepare("INSERT INTO global_stocks (name, size, price, global_stock_category_id, store_id) VALUES (?, ?, ?, ?, ?)");
    $stmt->bind_param("ssdis", $name, $size, $price, $category_id, $store_id);
...
if ($action === 'edit_stock') {
    ...
    if ($name != '' && $size != '') {
        $stmt = $koneksi->prepare("UPDATE global_stocks SET name = ?, size = ?, price = ?, global_stock_category_id = ? WHERE id = ? AND store_id = ?");
        $stmt->bind_param("ssdiis", $name, $size, $price, $category_id, $id, $store_id);
